...
def login_emt(config: Settings, minio_client: Minio, object_login_name: str) -> str:
    ...
    #expiration_date_unix = login_json['data'][0]['tokenDteExpiration']['$date']
...
def token_control(config: Settings, minio_client: Minio, date_slash: str, date_day: str) -> str:
    ...
    if not check_file_exists(minio_client, config.storage.config.minio.bucket, object_login_name):
        ...
        expiration_date = datetime.datetime.utcfromtimestamp(expiration_date_unix / 1000)  #  miliseconds to seconds
...
async def main(config: Settings):
    ...
    access_token = token_control(config, minio_client, formatted_date_slash, formatted_date_day)  # Obtain token from EMT
    ...
    async with aiohttp.ClientSession() as session:
        ...
        print("Errors in ETA AFTER RETRYING:", errors_eta_retry, "list of stops erroring AFTER RETRYING:", list_stops_error_retry)
